...
# parser.add_argument('--dataname', type=str, default='beijing', help='Name of dataset.')#  by zss
parser.add_argument('--dataname', type=str, default='adult', help='Name of dataset.')#  by zss

args = parser.parse_args()


def preprocess_adult():# by zss
    with open(f'{INFO_PATH}/adult.json', 'r') as f:
        info = json.load(f)

    data_path = info['raw_data_path']
    header = info['headers']
    data_df = pd.read_csv(data_path,header=None,names=header,na_values=['?'], sep=',\s',engine='python')
    columns = data_df.columns

    data_df.drop('fnlwgt', inplace=True, axis=1)#删除属性：'fnlwgt'

    df_cleaned = data_df.dropna()#去掉 nan的行
...
def process_data(name):
    ...
    if info['test_path']:
        ...
        with open(test_path, 'r') as f:
            ...
            test_save_path=info['save_test_path'] #by zss
    ...
    for col_idx in num_col_idx:
        print(train_df.columns[col_idx])
        col_info[col_idx] = {}
        col_info[col_idx]['type'] = 'numerical'# 修改col_info['type'] = 'numerical'

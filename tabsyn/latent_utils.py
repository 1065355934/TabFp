...
from tabsyn.vae.model import Decoder_model, Encoder_model
# from vae.model import Decoder_model, Encoder_model #  by zss
...
def compute_loss(X_num, X_cat, Recon_X_num, Recon_X_cat, mask_col=None):
    ...
    # wandb.log({'acc': acc.item() / total_num})# by zss
    ce_loss /= (idx + 1)
    # wandb.log({'ce_loss': ce_loss})# by zss
    # wandb.log({'mse_loss': mse_loss.item()})# by zss
...
def get_decoder_latent_train(X_num, X_cat, info, device, aux=None, mask_col=None):
    ...
    for i in range(1000):
        ...
        # wandb.log({'loss': loss.item()}) # by zss
        loss.backward()
        optimizer.step()
        scheduler.step(loss)
    return latent
...
def get_input_train(args):
    ...
    curr_dir = os.path.dirname(os.path.abspath(curr_dir))# by zss
    ...
    embedding_save_path = f'{curr_dir}/ckpt/{dataname}/train_z.npy' # by zss

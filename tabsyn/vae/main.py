...
def main(args):
    ...
    curr_dir = os.path.dirname(os.path.dirname(curr_dir))# revise by zss

    dataname = args.dataname
    #data_dir = f'{curr_dir}/../../data/{dataname}'# revise by zss
    data_dir = f'{curr_dir}/data/{dataname}'# revise by zss
    ...
    #info_path = f'{curr_dir}/../../data/{dataname}/info.json' # revise by zss
    info_path = f'{curr_dir}/data/{dataname}/info.json'# revise by zss
    ...
    with torch.no_grad():
        ...
        # Confirm the model weights are successfully loaded and saved
        print('Successfully load and save the model!')

        # Initialize a list to store batches of embeddings
        batched_embeddings = []
        ...
        n_batches = (X_train_num.shape[0] + batch_size - 1) // batch_size
        ...
        for i in range(n_batches):
            ...
            X_train_num_batch = X_train_num[start_idx:end_idx].to(device)
            X_train_cat_batch = X_train_cat[start_idx:end_idx].to(device)

            # Generate embeddings for the current batch and convert to numpy
            batch_z = pre_encoder(X_train_num_batch, X_train_cat_batch).detach().cpu().numpy()

            # Append the embeddings from the current batch to the list
            batched_embeddings.append(batch_z)

        # Concatenate all batch embeddings into a single array
        train_z = np.concatenate(batched_embeddings, axis=0)

        # Save the combined embeddings to disk
        np.save(f'{ckpt_dir}/train_z.npy', train_z)

        # Confirm the embeddings are successfully saved

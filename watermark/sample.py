...
def main(args, i):
    ...
    if num_samples == -1:
        # sample with the same number of training samples
    ...
    sample_dim = in_dim
    torch.manual_seed(i)#种子i作为不同用户密钥
    init_latents = torch.randn([num_samples, sample_dim], device=device)#标准正态分布（均值为0，方差为1，即高斯白噪声）
    ...
    if with_w == 'treering':
        ...
        latent_seed = torch.randint(0, 2, (init_latents.shape[1],1))# by zss
        ...
        adjusted_bit_string = adjusted_bit_string
        # Generate quantiles in vectorized manner for each condition
        latents_2[adjusted_bit_string == 0] = normal_dist.icdf(random_samples[adjusted_bit_string == 0] * 0.5)# 分布保持采样-->水印潜向量z_T^w
    ...
    for k in x_next_dict.keys():
        ...
        syn_num, syn_cat, syn_target = split_num_cat_target(syn_data, info, num_inverse, cat_inverse, args.device)#潜向量z_0^w解码为表格
